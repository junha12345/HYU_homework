...
class BankAccount {
public:
  BankAccount(const string& name, unsigned int balance, double interest_rate);
  virtual ~BankAccount() {};
...
public:
  SavingsAccount(const string& name, int balance, double interest_rate);
  virtual ~SavingsAccount() {};

  virtual const char* type() const { return "savings"; }
  // Compute compound interest rate
  virtual unsigned int compute_expected_balance(
